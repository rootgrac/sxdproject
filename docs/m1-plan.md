...
| M1-9 | 杀进程不丢档验证：调度层测试 + 引擎内手动验收（附录 D） | §4.1 DoD | S | ✅ 逻辑层集成用例（多关连推→新会话恢复 node/copper/level；章节完成持久化）；引擎内手动验收待装配预览（刷新页面→继续） |
...
- [x] 新档：创建 → 推图 → 升级 → 强化，全流程无阻塞（**2026-09-04 引擎内预览验收通过**）
- [x] 杀进程 / 断电重启，进度不丢（最多回退 30 秒）——逻辑层集成用例 + 引擎内刷新续档验证
- [x] 存档损坏自动回退备份并可继续游玩（KvSaveStore/Node 原型测试覆盖）
- [x] 战斗确定性回放一致（内核同种子一致性测试 + demo 复跑校验）
- [x] 旧版本存档 → 新版本迁移 fixtures 通过（v1→v2 迁移测试）
- [ ] 最低机型战斗 ≥30fps（待真机/原生构建轮）
